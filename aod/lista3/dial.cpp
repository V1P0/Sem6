  ...
  if (is_ss) {
    ...
    for (const auto &source : ss_sources->verticies) {
      auto start = std::chrono::high_resolution_clock::now();
            ...
            std::unique_ptr<algorithms::PriorityQueue> pq = std::make_unique<algorithms::DialPriorityQueue>(graph->max_edge,graph->getSize());
    ...
    for (const auto &[source, dest] : p2p_sources->search_pairs) {
      ...
      auto result = algorithms::runDijsktra(*graph, source, dest, pq);
      results.push_back(result);
      std::cout
